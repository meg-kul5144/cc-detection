...
def predict():
    ...
    LIMIT_BAL = scaler_bal.transform(np.array(float(request.values.get('Client credit'))).reshape(-1,1))
    ...
    repayment_score_earlymonths = scaler_earlymon.transform(np.array(repayment_score_earlymonths).reshape(-1,1))

    repayment_score_latemonths = pay_0 + pay_2 + pay_3
    repayment_score_latemonths = scaler_latemon.transform(np.array(repayment_score_latemonths).reshape(-1,1))
